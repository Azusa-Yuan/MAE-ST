...
def generate_data(args: argparse.Namespace):
    ...
    # read data
    data = np.load(data_file_path)["data"].astype(np.float64)
    ...
    if add_time_of_day:
        ...
        # mean, std = tod_tiled[..., 0].mean(), tod_tiled[..., 0].std()
        # tod_tiled = (tod_tiled-mean) / std
    ...
    if add_day_of_week:
        ...
        # mean, std = dow_tiled[..., 0].mean(), dow_tiled[..., 0].std()
        # dow_tiled = (dow_tiled - mean) / std
    ...
    if os.path.exists(args.graph_file_path):
        ...
        A = pd.read_csv("datasets/raw_data/NYC_TAXI/NYC_TAXI.csv", header=None).values.astype(np.float32)
        with open("datasets/raw_data/NYC_TAXI/adj_NYC_TAXI.pkl", "wb") as f:
            pickle.dump(A, f)
        # generate and copy
        generate_adj_pems08()
