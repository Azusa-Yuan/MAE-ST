...
def generate_data(args: argparse.Namespace):
    ...
    for file in files:
        if not os.path.isdir(file):
            if file.startswith('DA_'):
                data = pd.read_csv(data_file_path + '\\' + file).values
                if df_index is None:
                    df_index = data[:, 0:1]
    ...
    data = np.transpose(data_list, (2, 0, 1))
...
if __name__ == "__main__":
    ...
    DATASET_NAME = "Solar"      # sampling frequency: every 1 hour
    ...
    DATA_FILE_PATH = "datasets/raw_data/{0}".format(DATASET_NAME)
