...
class GraphWaveNet(nn.Module):
    ...
    def __init__(self, num_nodes, dropout=0.3, supports=None,
                 ...
                 kernel_size=2, blocks=4, layers=2, output_dim=1):
    ...
    def forward(self, history_data: torch.Tensor, pre_train_embedding, future_data: torch.Tensor = None,
        ...
        input = pre_train_embedding.transpose(1, 3).contiguous()

        B, dim, N, in_len = input.shape
        ...
        for i in range(self.blocks * self.layers):
            ...
            x = self.bn[i](x)
        # if pre_train_embedding is not None:
        #     addition = self.power *0.1 +self.fc_his(pre_train_embedding.permute(0, 2, 1, 3).reshape(B, N, -1))
        #     skip += addition.permute(0, 2, 1).unsqueeze(-1)
